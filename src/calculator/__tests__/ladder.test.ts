...
describe("exposure ladder", () => {
  it("50mm, cropFactor 1, static, no stabilisation → floor 1/50, ISO stays at isoBase until shutter reaches 1/50", () => {
    ...
    for (const light of LIGHTS) {
      for (const intent of INTENTS) {
        const s = solveExposure({ ...base, light, intent });
    ...
    const dim = solveExposure({ ...base, light: "indoor_dim", intent: "natural" });
  ...
  it("same lens, motion walking → floor 1/125, ISO rises earlier, floor.explain names subject motion", () => {
    const base = { focalMm: 50, body: FULL_FRAME, lens: FIFTY_PRIME, support: "handheld" as const, light: "night_street" as const, intent: "natural" as const };
      ...
      for (const intent of INTENTS) {
        for (const motion of MOTIONS) {
          for (const support of SUPPORTS) {
            const s = solveExposure({ light, intent, motion, support, focalMm: 55, body: FULL_FRAME, lens: KIT_ZOOM });
  ...
  it("unknown lens → aperture is null in the output, never a number", () => {
    for (const light of LIGHTS) {
      for (const intent of INTENTS) {
        for (const motion of MOTIONS) {
          for (const support of SUPPORTS) {
            const s = solveExposure({ light, intent, motion, support, focalMm: 50, body: FULL_FRAME, lens: UNKNOWN_LENS });
  ...
  it("isoMode locked at 100 in night_no_street → shortfallStops > 0 and shutterS never below floor.floorS", () => {
    ...
    for (const motion of MOTIONS) {
      for (const lens of [FIFTY_PRIME, UNKNOWN_LENS]) {
        const s = solveExposure({ light: "night_no_street", intent: "natural", motion, support: "handheld", focalMm: 50, body: locked, lens });
  ...
  it('tripod + static → floor.setBy === "tripod"', () => {
    ...
    const s = solveExposure({ light: "blue_hour", intent: "natural", focalMm: 50, body: FULL_FRAME, lens: FIFTY_PRIME, motion: "static", support: "tripod" });
            ...
            for (const body of bodies) {
              for (const lens of lenses) {
                for (const focalMm of [18, 35, 55]) {
                  const s = solveExposure({ light, intent, motion, support, body, lens, focalMm });
